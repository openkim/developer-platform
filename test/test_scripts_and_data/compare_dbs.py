...
def compare_db_to_reference(
    reference_json_path: os.PathLike,
    test_db: MontyDatabase,
    rtol: float = 0.01,
    atol: float = 0.01,
):
    """
    ...
    assert ref_db_len == test_db_len, (
        f"Database lengths are unequal. Ref. database has {ref_db_len} entries, "
        f"while test has {test_db_len}"
    )

    for i, ref_result in enumerate(ref_db.data.find({})):
        print(f"Processing reference result {i} of {ref_db_len}", end="\r")
            ...
            if isinstance(ref_result[key], dict):
                if ("source-value" in ref_result[key]) and (key not in KEYS_TO_SKIP):
                    ...
                    error_message_specifying_pair_and_key = (
                        f"\n\nTest failed while comparing to key '{key}' in "
                        f"instance-id {ref_instance_id} in reference "
                        f"runner-subject pair {ref_runner_and_subject}:\n"
                    )
                    ...
                    try:
                        test_source_value_array = np.asarray(
                            next(cursor)[key]["source-value"]
                        )
                    except StopIteration:
                        assert False, (
                            error_message_specifying_pair_and_key
                            + "No matches found in test DB."
                        )
                    except Exception:
                        raise RuntimeError(
                            "Unexpected exception when searching test DB"
                        )
                    ...
                    try:
                        ...
                        assert False, (
                            error_message_specifying_pair_and_key
                            + "Multiple matches found in test DB."
                        )
                    except StopIteration:
                        pass
                    except Exception:
                    ...
                    error_message_showing_source_values = (
                        ...
                        f"{ref_source_value_array}\n\nand "
                        f"test value\n\n{test_source_value_array}\n\n"
                    )

                    # arrays should be the same shape
                    assert (
                        ref_source_value_array.shape == test_source_value_array.shape
                    ), (
                        error_message_specifying_pair_and_key
                        + error_message_showing_source_values
                    ...
                    if np.issubdtype(ref_source_value_array.dtype, np.number):
                        assert np.allclose(
                            ...
                            error_message_specifying_pair_and_key
                            + error_message_showing_source_values
                            + "Numerical values are not within the "
                            "requested fractional tolerance."
                        )
...
if __name__ == "__main__":
    ...
    print(
        "SUCCESS! All results provided in reference database were successfully matched."
    )
